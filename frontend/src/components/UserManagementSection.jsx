...
const UserManagementSection = () => {
  ...
  const [newUser, setNewUser] = useState({ email: '', roles: ['user'], tenant_id: '' });
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      setNewUser({ email: '', roles: ['user'], tenant_id: '' });
      fetchUsers();
    } catch (err) {
  ...
  const handleRoleUpdate = async (userId) => {
    try {
      await api.put(`/users/${userId}`, { roles: editRoles });
      fetchUsers();
      setEditingUser(null);
      setEditRoles([]);
    } catch (err) {
  ...
  const startEditing = (u) => {
    setEditingUser(u.id);
    setEditRoles(u.role ? u.role.split(',').map(r => r.trim()) : []);
  };
            ...
            <form onSubmit={handleCreateUser} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mt-2">
                  {['user', 'project_manager', 'technical_manager', 'co_admin', 'admin'].map(role => {
                    if (role === 'admin' && !user.roles.includes('admin')) return null; // Only admin can assign admin
                    return (
                      <label key={role} className="inline-flex items-center">
                        ...
                        />
                        <span className="ml-2 text-sm text-slate-700 capitalize">{role.replace('_', ' ')}</span>
                      </label>
                    );
                  })}
                </div>
                      ...
                      {editingUser === u.id ? (
                        <div className="flex flex-col gap-2 min-w-[200px]">
                          <div className="flex flex-wrap gap-2">
                            {['user', 'project_manager', 'technical_manager', 'co_admin', 'admin'].map(role => {
                               if (role === 'admin' && !user.roles.includes('admin')) return null;
                               return (
                                <label key={role} className="inline-flex items-center">
                                  <input
                                    type="checkbox"
                                  ...
                                  />
                                  <span className="ml-1 text-xs text-slate-700 capitalize">{role.replace('_', ' ')}</span>
                                </label>
                               );
                            })}
                          </div>
                          <div className="flex gap-2 mt-1">
                            ...
                            <button 
                              ...
                              className="text-xs bg-white border border-slate-300 text-slate-700 px-2 py-1 rounded hover:bg-slate-50"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-1 items-center">
                          {u.role ? u.role.split(',').map(r => r.trim()).map(role => (
                            <span key={role} className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset ${
                              role === 'admin' ? 'bg-purple-50 text-purple-700 ring-purple-700/10' :
                          ...
                          {user.roles && user.roles.some(r => ['admin', 'co_admin'].includes(r)) && (
                            <button 
                              ...
                              title="Edit Roles"
                              disabled={user.roles.includes('co_admin') && u.role && u.role.includes('admin')}
                            >
